...
module.exports.registerSensor = (id, name, opts)=>{
  ...
  return new Promise((resolve, reject)=>{
    ...
    client.publish(`homeassistant/sensor/${device_name}_${id}/config`, JSON.stringify({...payload,...opts}), { qos: 1, retain: true }, (error, packet)=>{
      if(error) reject(error)
      resolve()
    })
  })
...
module.exports.sendSensorValue = (id, value, retain = false)=>{
  ...
  let device_name = DEVICE_NAME?.toLowerCase()?.replace(/ /g, '_')
  return new Promise((resolve, reject)=>{
    client.publish(`${device_name}/${id}/state`, value, { qos: 1, retain: retain }, (error)=>{
      if(error) reject(error)
      resolve()
    })
  })
